...
from app.modules.ai.dataset import MultispectralDataset
from app.modules.ai.model import CNNModel, save_model
...
def read_soil_moisture():
    df = pd.read_csv(r'C:\Users\HEW\Projekte\5gla-ai-spectral-unmixing\data\Bodenfeuchte\Ostfalia011.csv')
...
def prepare_images(images):
    ...
    images = [resize(image,(image.shape[0] // 4, image.shape[1] // 4, image.shape[2])) for image in images]
...
if __name__ == "__main__":
    ...
    val_mse = train_torch(image_folder=image_folder1, image_folder2=image_folder2, df=df,
                          batch_size=16, num_epochs=1000, learning_rate=0.000001, test_size=0.2,
                          assumed_capillarity=0.17, verbose=True)
    #
